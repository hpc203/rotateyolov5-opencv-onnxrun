...
private:
	const float anchors[3][10] = { {27, 26, 20, 40, 44, 19, 34, 34, 25, 47},
...
void YOLO::detect(Mat& frame)
{
	...
	int n = 0, q = 0, i = 0, j = 0, row_ind = 0; ///xmin,ymin,xamx,ymax, cos, sin, box_score,class_score
	float* pdata = (float*)outs[0].data;
	for (n = 0; n < 3; n++)   ///ÌØÕ÷Í¼³ß¶È
	{
		int num_grid_x = (int)(this->inpWidth / this->stride[n]);
		int num_grid_y = (int)(this->inpHeight / this->stride[n]);
		for (q = 0; q < 5; q++)    ///anchor
		{
			...
			for (i = 0; i < num_grid_y; i++)
			{
				for (j = 0; j < num_grid_x; j++)
				{
					...
					{
						Mat scores = outs[0].row(row_ind).colRange(7, 7 + this->num_class);
						Point classIdPoint;
						...
						{ 
							float cx = (sigmoid(pdata[0]) * 2.f - 0.5f + j) * this->stride[n];  ///cx
							float cy = (sigmoid(pdata[1]) * 2.f - 0.5f + i) * this->stride[n];   ///cy
							...
							///float angle = atanf(tanh(pdata[5]) / sigmoid(pdata[4])) * 180 / PI;
							float angle = acos(sigmoid(pdata[4])) * 180 / PI;
...
int main()
{
	...
	string imgpath = "images/1070.png";
